...
test_path = Path("cropped_test")
feat_path = Path('gabor_selected_indices') / Path("gabor_index_1000.csv")
root_output = Path("ML_Results")
...
for name, clf in zip(names, classifiers):
    ...
    train_metrics.to_csv(Path(root_output) / Path(name + "_train_metrics.csv"))
    ...
    # print(val_metrics)
    val_metrics.to_csv((root_output / Path(name + "_val_metrics.csv")))
    ...
    print(test_metrics)
    test_metrics.to_csv((root_output / Path(name + "_test_metrics.csv")))
